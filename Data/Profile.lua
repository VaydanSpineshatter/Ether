...
local function RefreshLayout(data)
    ...
    for _,button in pairs(data) do
        ...
        if button.healthBar then
            button.healthBar:SetStatusBarTexture((Ether.DB[811][3] or unpack(Ether.media.blankBar)))
        end
    end
end

local function CreateSections(tbl)
    Ether:CreatePositionSection(tbl)
    Ether:CreateCustomSection(tbl)
end

local function ProfileRefresh(tbl)
